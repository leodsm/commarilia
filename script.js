﻿document.addEventListener('DOMContentLoaded', function () {
    ...
    function ensureInitialNavigation(swiperInstance) {
        ...
        const renderedStories = newsData.filter(newsItem => selectedCategory === 'Todas' || newsItem.category === selectedCategory);
        const targetStoryRealIndex = renderedStories.findIndex(story => story.storyId === initialStoryId);

        if (targetStoryRealIndex < 0) {
            hasAppliedInitialNavigation = true;
            return;
        }

        swiperInstance.slideToLoop(targetStoryRealIndex, 0);
        
        const targetSlide = swiperInstance.slides[swiperInstance.activeIndex];
        const horizontalSwiper = initHorizontalSwiper(targetSlide); // Ensure it's initialized
        ...
        const resolvedSegmentId = initialSegmentId || getActiveSegmentId(horizontalSwiper);
    ...
    function createSegmentHTML(segment, newsIndex, segmentIndex) {
        ...
        // Geração de conteúdo comum (título, descrição, botão)
        ...
        if (segment.showButton) {
            ...
            contentBlocks.push(`<button class="read-more-btn btn-glass font-bold text-sm self-start transition-transform hover:scale-105" ${buttonBottomSpacing} data-news-index="${newsIndex}" data-segment-index="${segmentIndex}" data-segment-id="${segment.segmentId}"><span>Leia Mais</span></button>`);
        }
        const infoMarkup = contentBlocks.length ? `<div class="flex flex-col gap-3">${contentBlocks.join('')}</div>` : '';

        // Lógica de mídia separada para imagens e vídeos
        let mediaMarkup;
        if (isVideo) {
            // Para vídeos, mantemos o comportamento original (tela cheia) para melhor desempenho.
            // A parte superior fica sob o cabeçalho, mas é uma troca necessária.
            mediaMarkup = `
            ...
            // Para imagens, usamos a técnica de fundo desfocado + imagem contida.
            mediaMarkup = `
                <!-- Fundo desfocado para o efeito de continuidade no header -->
                <div class="absolute inset-0 w-full h-full bg-cover bg-center blur-md scale-110" 
                     style="background-image: url('${segment.mediaUrl}')">
                </div>
                <!-- Imagem principal, totalmente visível na área de conteúdo -->
                <div class="absolute left-0 right-0 bottom-0" style="top: 54px;">
                    <img class="w-full h-full object-contain" src="${segment.mediaUrl}" alt="">
                </div>
                ${segment.showOverlay ? '<div class="absolute left-0 right-0 bottom-0 bg-black/40" style="top: 54px;"></div>' : ''}
            `;
        }

        // Montagem final do slide
        return `
            <div class="swiper-slide bg-black" data-segment-id="${segment.segmentId}">
                ${mediaMarkup}
                <div class="z-10 px-6 pt-[70px] pb-6 flex flex-col ${contentPositionClass} ${bottomOffsetClass} h-full w-full text-left">
                    ${infoMarkup}
                </div>
            </div>
        `;
    ...
    function initHorizontalSwiper(slideElement) {
        if (!slideElement) return null;
        const horizontalContainer = slideElement.querySelector('.swiper-container-h');
        if (horizontalContainer && !horizontalContainer.swiper) {
            const storyId = slideElement.dataset.storyId;
            return new Swiper(horizontalContainer, {
                pagination: { el: horizontalContainer.querySelector('.swiper-pagination'), clickable: true },
                speed: 500,
                on: {
                    slideChange() { handleHorizontalChange(storyId, this); }
                }
            });
        }
        return horizontalContainer?.swiper || null;
    }
    ...
    function initSwipers() {
        ...
        mainSwiper = new Swiper('.swiper-container-v', {
            ...
            on: {
                init: function () {
                    initHorizontalSwiper(this.slides[this.activeIndex]);
    ...
    function setupEventListeners() {
        ...
        if (onboardingOverlay) { // Use the new function
            onboardingOverlay.addEventListener('click', hideOnboarding);
        }

        // Keyboard Navigation
        document.addEventListener('keydown', (e) => {
            // If modal is open, only 'Escape' works to close it.
            if (!modal.classList.contains('invisible')) {
                if (e.key === 'Escape') closeModal();
                return;
            }

            // If onboarding is visible, 'Escape' or 'Enter' will close it.
            if (onboardingOverlay && !onboardingOverlay.classList.contains('hidden')) {
                if (e.key === 'Escape' || e.key === 'Enter') hideOnboarding();
                return;
            }

            if (!mainSwiper) return;
            const activeHorizontalSwiper = mainSwiper.slides[mainSwiper.activeIndex]?.querySelector('.swiper-container-h')?.swiper;

            switch (e.key) {
                case 'ArrowRight':
                    activeHorizontalSwiper?.slideNext();
                    break;
                case 'ArrowLeft':
                    activeHorizontalSwiper?.slidePrev();
                    break;
                case 'ArrowDown':
