                ...
                {allCategories.map(cat => (
                    <SwiperSlide key={cat} className="!w-auto flex items-center">
                        <button
                            ...
                            className={`category-link inline-block text-white/80 text-xs py-1 px-3 rounded-full bg-white/10 backdrop-blur-sm transition-all duration-300 ${activeCategory === cat ? 'active' : ''}`}
                        >
                            {cat}
                        </button>
                    </SwiperSlide>
